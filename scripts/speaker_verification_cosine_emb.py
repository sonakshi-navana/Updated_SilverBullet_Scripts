    ...
    with torch.no_grad():
        for batch in tqdm(data_loader, dynamic_ncols=True):
            ...
            for i, seg_id in enumerate(seg_ids):
                embedding_dict[seg_id] = emb[i].detach().clone().cpu().numpy()
...
def get_verification_scores(veri_test):
    ...
    for i, line in enumerate(veri_test):
        ...
        enrol_id = "_".join(line.split(" ")[1].rstrip().split(".")[0].strip().split("/")[-2:])
        # enrol_id = line.split(" ")[1].rstrip().split(".")[0].strip()
        test_id =  "_".join(line.split(" ")[2].rstrip().split(".")[0].strip().split("/")[-2:])
        # test_id = line.split(" ")[2].rstrip().split(".")[0].strip()
        #print(enrol_dict,test_dict)
        #print(enrol_id, test_id)
...
def dataio_prep(params):
    ...
    val_data = sb.dataio.dataset.DynamicItemDataset.from_csv(
        csv_path=params["val_data"], replacements={"data_root": data_folder},
    )
    val_data = val_data.filtered_sorted(sort_key="duration")

    print(val_data)

    # UNVAL data
    unval_data = sb.dataio.dataset.DynamicItemDataset.from_csv(
        csv_path=params["unval_data"], replacements={"data_root": data_folder},
    )
    unval_data = unval_data.filtered_sorted(sort_key="duration")

    datasets = [unval_data, val_data]
    print(unval_data)
    ...
    val_dataloader = sb.dataio.dataloader.make_dataloader(
        val_data, **params["val_dataloader_opts"]
    )

    unval_dataloader = sb.dataio.dataloader.make_dataloader(
        unval_data, **params["unval_dataloader_opts"]
    )

    return val_dataloader, unval_dataloader
...
if __name__ == "__main__":
    ...
    from voxceleb_prepare_mr_d1 import prepare_voxceleb  # noqa E402
    ...
    #train_dataloader, enrol_dataloader, test_dataloader, dev_dataloader = dataio_prep(params)

    val_dataloader, unval_dataloader = dataio_prep(params)
    ...
    # Computing  dev/train embeddings
    logger.info("Computing dev/train embeddings...")
    dev_dict = compute_embedding_loop(dev_dataloader)
    #first run
    train_dict = compute_embedding_loop(train_dataloader) """
    
    """ #print(len(enrol_dict))
    print("test",len(test_dict))
    print("dev",len(dev_dict))
    print("train",len(train_dict))
    print(test_dict)
    #print(enrol_dict["16794060_281474989444559"]) """

    


    # Compute the val embeddings! - Block 1 
    logger.info("Computing val embeddings...")
    #First Run 
    val_dict = compute_embedding_loop(val_dataloader)
    #print(val_dict)
    import pickle
    directory = '/mnt/training/silverbullet-testenv/test-01-bigexp/save/'
    # open a file to save the data
    with open(directory+'val_dict1.pickle', 'wb') as f:
    ...
    # Compute the unval embeddings! - Block 2 
    logger.info("Computing unval embeddings...")
    #First Run 
    unval_dict = compute_embedding_loop(unval_dataloader)
    #print(val_dict)
    import pickle

    directory = '/mnt/training/silverbullet-testenv/test-01-bigexp/save/'
    # open a file to save the data
    with open(directory+'unval_dict1.pickle', 'wb') as f:
    # write the data to the file using pickle.dump()
        pickle.dump(unval_dict, f) 
